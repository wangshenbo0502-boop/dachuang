...
> **⚠️ 历史快照声明（2026-09-13）**：本文件为 v1.1 时点（2026-08-14）的后端进度快照，**保留作历史记录，不作为当前进度与契约**。此后项目已发生如下变化（本快照未包含）：
> 1. 新增 SSE 流式接口（`app/api/streaming.py`）、`GET /api/usage` Token 用量接口、滑动窗口限流中间件——文中"仍待改进"所列限流/SSE/Token 统计等项**现已实现**；
> 2. 前端已完成 React 19 + Three.js 3D 沉浸式改造，岗位匹配房间/档案编辑/画像分析已接 API；
> 3. 文中第九章接口清单为 v1.1 时点快照，与当前实际注册的 **27 个**接口不一致（缺 SSE 流式 3 个、`GET /api/usage` 等），以 [docs/02-API接口文档](../docs/02-API接口文档.md) 为准。
>
> 当前进度以 [docs/PROGRESS.md](../docs/PROGRESS.md) 为准，接口契约以 [docs/02-API接口文档](../docs/02-API接口文档.md) 为准。另注意：第七章启动步骤中的 `python main.py` **不能启动服务**（`main.py` 无启动块），须用 `uvicorn main:app`（见 [docs/06-部署说明](../docs/06-部署说明.md)）。
