...
export const api={
 ...
 resume:(b:object)=>apiData<T.ResumeResponse>(request.post("/resume",b)), resumeDetail:(id:number)=>apiData<T.ResumeResponse>(request.get(`/resume/${id}`)), resumes:(id:number)=>apiData<T.ResumeHistory[]>(request.get(`/resume/user/${id}`)),
 ...
 chatProfileTurn:(b:object)=>apiData<T.ChatTurnResponse>(request.post("/chat/profile-turn",b)),
 ...
 syncChatProfile:(b:object)=>apiData<T.StudentProfile>(request.post("/chat/sync-profile",b)),
