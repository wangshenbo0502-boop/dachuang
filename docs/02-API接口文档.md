...
> **V2.2 变更说明（2026-09-13）**：依据当前代码（`backend/main.py`、`backend/app/api/*`、`frontend/vite.config.ts`）核对全文。第 3-9 章全部 27 个接口契约与代码一致，未做改动；仅修正第 10 章联调约定中"vite 代理被注释"的过时描述（代理现已默认启用）。已知偏差（404 资源不存在实际返回 `code=3001`）仍保留在 2.2 节，未修复。
